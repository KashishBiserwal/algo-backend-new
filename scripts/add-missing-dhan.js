      ...
      fs.createReadStream(csvPath)
        ...
        .on('data', async (row) => {
          ...
          if (processedCount % 10000 === 0) {
            console.log(`📊 Processed ${processedCount} records (${addedCount} added, ${skippedCount} skipped)`);
          }
          ...
          const exists = await Instrument.findById(universalId);
          if (exists) {
            skippedCount++;
            return;
          }
          ...
          batch.push(instrumentData);
          
          // Process batch when it reaches batchSize
          if (batch.length >= batchSize) {
            try {
              await Instrument.insertMany(batch, { ordered: false });
              addedCount += batch.length;
              console.log(`✅ Added batch of ${batch.length} instruments`);
            } catch (error) {
              // Count successful inserts from error
              if (error.writeErrors) {
                const successful = batch.length - error.writeErrors.length;
                addedCount += successful;
                console.log(`⚠️ Batch partially successful: ${successful}/${batch.length} added`);
              }
            }
        ...
        .on('end', async () => {
          ...
          if (batch.length > 0) {
            try {
              await Instrument.insertMany(batch, { ordered: false });
              addedCount += batch.length;
              console.log(`✅ Added final batch of ${batch.length} instruments`);
            } catch (error) {
              if (error.writeErrors) {
                const successful = batch.length - error.writeErrors.length;
                addedCount += successful;
                console.log(`⚠️ Final batch partially successful: ${successful}/${batch.length} added`);
              }
            }
          }
